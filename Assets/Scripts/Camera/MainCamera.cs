...
public class MainCamera : MonoBehaviour {
    ...
    private void Start()
    {
        transform.position = player.transform.position + Vector3.forward * z;
    ...
    private void FixedUpdate()
    {
        if (isUnderPlayerControl)
        {
            if (player.isAiming)
            {
                ...
                transform.position = (Vector3)(Vector2.Lerp(transform.position, target, Time.fixedDeltaTime * moveLerpSpeed) + kick) + Vector3.forward * z;
            }
            else
            {
                transform.position = (Vector3)(Vector2.Lerp(transform.position, player.transform.position, Time.fixedDeltaTime * moveLerpSpeed) + kick) + Vector3.forward * z;
            }
        }
    }
